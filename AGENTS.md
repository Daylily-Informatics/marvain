...
source marvain_activate
```

- Never work around terminal or activation problems by skipping `source marvain_activate`.
...
- Use the Marvain CLI as the primary interface: prefer `marvain ...` or `./bin/marvain ...` over raw AWS, SAM, direct Python module execution, or direct config edits.
...
- `marvain_activate` is for activating the Conda env and making the repo CLI available. Do not move runtime bootstrap, config creation, AWS deployment, or unrelated tool installation into activation.
- Python package dependencies belong in `pyproject.toml`.
- Conda/system bootstrap belongs in `config/marvain_conda.yaml`.
