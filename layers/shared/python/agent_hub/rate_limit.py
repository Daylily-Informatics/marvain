            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt >= max_retries:
                        logger.error("Max retries (%d) exceeded for %s: %s", max_retries, func.__name__, str(e))
                        raise RateLimitError(f"Max retries exceeded: {str(e)}") from e
                    ...
                    logger.warning(
                        "Retry %d/%d for %s after %.2fs: %s", attempt + 1, max_retries, func.__name__, delay, str(e)
                    )
                    time.sleep(delay)
