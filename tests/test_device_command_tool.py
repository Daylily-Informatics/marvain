...
class TestDeviceCommandHandler:
    ...
    def test_device_not_connected_returns_error(self, mock_get_connections):
        ...
        ctx = MockToolContext()
        ctx.db.query.return_value = [{"device_id": "device-xyz", "agent_id": "agent-123", "name": "Test Device"}]
    ...
    def test_ws_endpoint_not_configured_returns_error(self):
        ...
        try:
            ctx = MockToolContext()
            ctx.db.query.return_value = [{"device_id": "device-xyz", "agent_id": "agent-123", "name": "Test"}]
...
class TestSendToConnection:
    ...
    def test_successful_send_returns_true(self, mock_boto_client):
        ...
        result = _send_to_connection(
            "conn-123", {"type": "cmd.ping"}, "https://test.execute-api.us-east-1.amazonaws.com/prod"
        )
